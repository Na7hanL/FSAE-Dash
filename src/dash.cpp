...
void canSniff(const CAN_message_t &msg) {
  ...
  switch(msg.id) { // ID's 1520+ are Megasquirt CAN broadcast frames
    ...
    case 1520: // group 0
      rpm_dash = (int)(word(msg.buf[6], rxmsg.buf[7]));
    break;
    
    case 1521: // group 1
      afrtgt_dash = (int)(word(0x00, msg.buf[4]));
    break;

    case 1522: // group 2
      clt_dash = (int)(word(msg.buf[6], msg.buf[7]));
    break;

    case 1523: // group 3
      bat_dash = (int)(word(msg.buf[2], msg.buf[3]));
    break;

    case 1551: // group 31
      afr_dash = (int)(word(0x00, msg.buf[0]));
    break;

    case 1553: // group 33
      gear_dash = (int)(word(0x00, msg.buf[6]));
    break;

    case 1562: // group 42
      spd_dash = (int)(word(msg.buf[0], msg.buf[1]));
    break;

    case 1563: // group 43
      sync_dash = (int)(word(0x00, msg.buf[0]));
    break;
...
uint16_t Add_Diag_To_Display_List_dash(uint16_t FWol){
    ...
    FWol = EVE_PrintF(FWol, 247, 355, 25, EVE_OPT_CENTER, "AFR:  %3d", afr_dash);

    FWol = EVE_PrintF(FWol, 567, 130, 25, EVE_OPT_CENTER, "CLT:  %3d", clt_dash);

    FWol = EVE_PrintF(FWol, 553, 205, 25, EVE_OPT_CENTER, "GEAR:  %3d", gear_dash);

    FWol = EVE_PrintF(FWol, 542, 280, 25, EVE_OPT_CENTER, "SPEED:  %3d", spd_dash);

    FWol = EVE_PrintF(FWol, 550, 355, 25, EVE_OPT_CENTER, "SYNC:  %3d", sync_dash);
...
void shiftUp() {
  ...
  if(oldGear == gear_dash){ // if the shift didn't occur, aka gear before = gear after
    if(oldGear != 6){
      if(shiftTry < REJTRY){
...
void shiftDown() {
  oldGear = gear_dash;
  if(rpm_dash < U_L_RPM) { //if RPM too high, don't even attempt to shift
...
void shiftNeutral() {
  ...
  if (gear_dash == oldGear){ //if the shift didn't work, try again
    if(oldGear != 6){
      if(shiftTry < REJTRY){
        shiftNeutral(); 
        shiftTry = shiftTry + 1; //keep a count of how many tries it took
...
uint16_t Add_Dash_To_Display_List(uint16_t FWol, bool dashMode)
{
    if(dashMode){
        ...
        FWol = EVE_PrintF(FWol, 300, 200, 25, EVE_OPT_CENTER, "%3d", rpm_dash);
        ...
        FWol = Add_Diag_To_Display_List_dash(FWol);
